...
namespace Verdict.UI.Narrative
{
    public sealed class ScreenFadePresenter : MonoBehaviour
    {
        ...
        public void Trigger(string colorName, float duration)
        {
            ...
            fadeRoutine = StartCoroutine(FadeRoutine(targetColor, Mathf.Max(0.05f, duration > 0f ? duration : defaultDuration)));
        }
        ...
        private IEnumerator FadeRoutine(Color targetColor, float duration)
        {
            ...
            while (elapsed < duration)
            {
                elapsed += Time.deltaTime;
                float t = Mathf.Clamp01(elapsed / duration);
                fadeImage.color = Color.Lerp(targetColor, fadeOutColor, t);
                yield return null;
            }
