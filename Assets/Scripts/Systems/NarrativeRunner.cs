...
namespace Verdict.Systems
{
    ...
    /// Owns no UI. Owns no gameplay beyond reading CourtState for
    /// condition nodes. Only advances a NarrativeRuntime.
    /// </summary>
    public sealed class NarrativeRunner
    {
        private readonly CourtStateRuntime courtState;

        public NarrativeRunner(CourtStateRuntime courtState)
        {
        ...
        private bool EvaluateCondition(NarrativeConditionData condition)
        {
            ...
            int current = courtState.GetCourtStat(condition.Stat);
